    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-14 reveal">
          ...
          <h2 id="portfolio-heading" className="section-title">{sectionLabel}</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-8">
          {portfolio.map((item, idx) => {
            ...
            const cardStyle: React.CSSProperties = { borderColor: 'var(--color-border)', backgroundColor: 'white' };
            ...
            const inner = (
              <>
                ...
                {/* Content */}
                <div className="p-6">
                  <h3 className="font-semibold text-base mb-2 transition-colors duration-200" style={{ color: 'var(--color-text)' }}>
                    {item.title}
                  </h3>
                  <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--color-text-3)' }}>
                    {item.description}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {item.tags.map((tag) => (
                      <span key={tag} className="tag-badge-gray">{tag}</span>
                    ))}
                  </div>
                </div>
              </>
            ...
            return item.linkUrl ? (
              <a key={item.title} href={item.linkUrl} target="_blank" rel="noopener noreferrer"
                 aria-label={`View ${item.title} example site`} className={cardClass} style={cardStyle}>
                {inner}
              </a>
            ) : (
